...
# -- Proprietary modules -- #
from utils import GROUP_NAMES, ICE_STRINGS
...
def compute_metrics(true, pred, charts, metrics):
    ...
    for chart in charts:
        if true[chart].ndim == 1 and pred[chart].ndim == 1:
            scores[chart] = np.round(
                metrics[chart]["func"](true=true[chart], pred=pred[chart]) * 100, 3
            )
        else:
            print(
                f"true and pred must be 1D numpy array, "
                f"got {true['SIC'].ndim} and {pred['SIC'].ndim}"
                f"dimensions with shape {true['SIC'].shape} and {pred['SIC'].shape}, respectively"
            )
