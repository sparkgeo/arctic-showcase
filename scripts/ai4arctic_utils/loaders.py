...
class AI4ArcticChallengeDataset(Dataset):
    ...
    def random_crop(self, scene):
        ...
        patch = np.zeros(
            (
                len(self.options["full_variables"]) + len(self.options["amsrenv_variables"]),
        ...
        if (
            ...
            patch[len(self.options["full_variables"]) :, :, :] = (
                torch.nn.functional.interpolate(
                    input=torch.from_numpy(
                        ...
                        .values[
                            :,
                            int(amsrenv_row) : int(
                                amsrenv_row + np.ceil(self.options["amsrenv_patch"])
                            ),
                            int(amsrenv_col) : int(
                                amsrenv_col + np.ceil(self.options["amsrenv_patch"])
                            ),
                        ]
                    ).unsqueeze(0),
    ...
    def prep_dataset(self, patches):
        ...
        ----------
        patches : ndarray
            Patches sampled from ASID3 ready-to-train challenge dataset
            scenes [PATCH, CHANNEL, H, W].
    ...
    def __getitem__(self, idx):
        ...
        while sample_n < self.options["batch_size"]:
            ...
            scene = xr.open_dataset(
                os.path.join(self.options["path_to_processed_data"], self.files[scene_id])
            )
            # - Extract patches
            try:
                ...
                print(
                    ...
                    f" ({self.options['patch_size']}, {self.options['patch_size']})"
                )
...
class AI4ArcticChallengeTestDataset(Dataset):
    """Pytorch dataset for loading full scenes from the ASID
    ready-to-train challenge dataset for inference."""
    ...
    def prep_scene(self, scene):
        """
        Upsample low resolution to match charts and SAR resolution.
        Convert patches from 4D numpy array to 4D torch tensor.
        ...
        x :
            4D torch tensor, ready training data.
        y :
            Dict with 3D torch tensors for each reference chart; reference inference data for x.
            None if test is true.
        """
        x = torch.cat(
            (
                torch.from_numpy(scene[self.options["sar_variables"]].to_array().values).unsqueeze(
                    0
                ),
                torch.nn.functional.interpolate(
                    input=torch.from_numpy(
                        scene[self.options["amsrenv_variables"]].to_array().values
                    ).unsqueeze(0),
    ...
    def __getitem__(self, idx):
        ...
        y :
            ...
            None if test is true.
        masks :
            Dict with 2D torch tensors; mask for each chart for loss calculation.
            Contain only SAR mask if test is true.
        name : str
            Name of scene.

        """
        scene = xr.open_dataset(
            os.path.join(self.options["path_to_processed_data"], self.files[idx])
        )
...
def get_variable_options(train_options: dict):
    ...
    train_options["amsrenv_upsample_shape"] = (
        int(
            ...
            + train_options["amsrenv_patch_dec"] * train_options["amsrenv_delta"]
        ),
        int(
            train_options["patch_size"]
            + train_options["amsrenv_patch_dec"] * train_options["amsrenv_delta"]
