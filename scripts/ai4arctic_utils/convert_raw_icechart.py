...
def convert_polygon_icechart(scene):
    ...
    converted_codes[:, SIC_LOOKUP["sic_partial_idx"][0]][ice_ct_ca_empty] = converted_codes[
        :, SIC_LOOKUP["total_sic_idx"]
    ][ice_ct_ca_empty]
    ...
    sod_a_b_bool = (
        converted_codes[:, SOD_LOOKUP["sod_partial_idx"][0]]
    ...
    floe_a_b_bool = (
        converted_codes[:, FLOE_LOOKUP["floe_partial_idx"][0]]
    ...
    sod_a_b_bool[
        np.where(converted_codes[:, SOD_LOOKUP["sod_partial_idx"][0]] == ICECHART_NOT_FILLED_VALUE)
    ] = False
    sod_a_c_bool[
        np.where(converted_codes[:, SOD_LOOKUP["sod_partial_idx"][0]] == ICECHART_NOT_FILLED_VALUE)
    ] = False
    sod_b_c_bool[
        np.where(converted_codes[:, SOD_LOOKUP["sod_partial_idx"][1]] == ICECHART_NOT_FILLED_VALUE)
    ] = False
    floe_a_b_bool[
        np.where(
            converted_codes[:, FLOE_LOOKUP["floe_partial_idx"][0]] == ICECHART_NOT_FILLED_VALUE
        )
    ] = False
    floe_a_c_bool[
        np.where(
    ...
    floe_b_c_bool[
        np.where(
            converted_codes[:, FLOE_LOOKUP["floe_partial_idx"][1]] == ICECHART_NOT_FILLED_VALUE
        )
    ] = False
    ...
    # Loop to find
    for idx, (compare_idx, sod_bool, floe_bool) in enumerate(
        zip(compare_indexes, sod_bool_list, floe_bool_list)
    ):
        ...
        if tmp_sod_bool_indexes.size:  # i.e. is array is not empty.
            sod_partial_add[tmp_sod_bool_indexes, SIC_LOOKUP["sic_partial_idx"][compare_idx[0]]] = (
                converted_codes[:, SIC_LOOKUP["sic_partial_idx"][compare_idx[1]]][
                    tmp_sod_bool_indexes
                ]
            )

        if tmp_floe_bool_indexes.size:  # i.e. is array is not empty.
            floe_partial_add[
                tmp_floe_bool_indexes, SIC_LOOKUP["sic_partial_idx"][compare_idx[0]]
            ] = converted_codes[:, SIC_LOOKUP["sic_partial_idx"][compare_idx[1]]][
                tmp_floe_bool_indexes
            ]
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        for i in range(codes.shape[0]):
            ...
            if (
                np.divide(
                    np.max(tmp_sod_added[i, SIC_LOOKUP["sic_partial_idx"]]),
                    tmp_sod_added[i, SIC_LOOKUP["total_sic_idx"]],
                )
                * 100
                >= SOD_LOOKUP["threshold"] * 100
            ):
            ...
            if (
                np.divide(
                    np.max(tmp_floe_added[i, SIC_LOOKUP["sic_partial_idx"]]),
                    tmp_floe_added[i, SIC_LOOKUP["total_sic_idx"]],
                )
                * 100
                >= FLOE_LOOKUP["threshold"] * 100
            ):
            ...
            if any(
                converted_codes[i, FLOE_LOOKUP["floe_partial_idx"]] == FLOE_LOOKUP["fastice_class"]
            ):
