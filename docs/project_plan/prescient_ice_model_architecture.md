...
Clay's expected inputs are: a pixel tensor `[batch, bands, height, width]`, a wavelengths tensor (nm) describing the spectral content of each band, and a timestamps tensor encoding `[week, hour, lat, lon]` (zeros are acceptable for cases where temporal/spatial position is not used). Normalisation is per-band `(pixels - mean) / std` using values supplied in the model's metadata configuration. Clay is sensor-agnostic and accepts custom sensor metadata entries with custom band names, GSD, wavelengths, and normalisation statistics.
...
Implementation cost for Configuration 3 over Configuration 2 is negligible — the class token is the first element of the encoder output that already contains the patch tokens — so both Clay-based configurations can be evaluated in parallel without significant additional cost.
...
1. **Absolute performance shortfall.** Phase 1 R² on held-out scenes falls below a threshold T. T is to be set by reviewing the full AutoICE results distribution (Stokholm et al., 2024, *The Cryosphere*, 18, 3471–3494) before Phase 1 evaluation begins — not solely the winning score, which sets an upper bound but does not reflect the realistic range of well-engineered submissions. Calibrating T against the broader distribution sets a more honest bar.
...
- **Clay patch token API call.** The specific Clay v1.5 codebase API for accessing the full `[batch, 1025, 1024]` encoder output tensor — and the canonical convention that the first sequence element is the class token (chip embedding) with the remaining 1024 being patch tokens — needs to be confirmed against the v1.5 source. Empirical testing on Sentinel-2 inputs has confirmed the tensor shape and the apparent ordering; the architectural commitment to patch tokens and class token is firm.
- **Clay v1.5 embedding dimension.** Confirmed at 1024 for both patch tokens and the class token chip embedding, based on empirical testing on Sentinel-2 inputs. The same dimension is expected for Sentinel-1 EW inputs under the custom `sentinel-1-ew` metadata entry, since Clay v1.5 is sensor-agnostic and shares the encoder embedding space across sensors.
...
- **Phase 2 threshold T.** To be set after reviewing the full AutoICE results distribution before Phase 1 evaluation begins.
