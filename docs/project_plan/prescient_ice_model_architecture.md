...
The land mask itself is a project asset registered in Prescient as a single STAC collection (`land-mask`) in EPSG:3978, derived from an authoritative coastline source (GSHHG full-resolution or OSM coastlines are the standard candidates for Arctic work). For AI4Arctic training data specifically, the bundled `distance_map` variable already encodes per-pixel distance-to-land with land marked as code `1`, so the AI4Arctic training path derives its land mask directly from the NetCDF rather than from the Prescient `land-mask` collection. This keeps training and inference symmetric in their treatment of land at Stage 3 — both apply land masking at chip preparation in the same way, just sourced from different rasters consistent with each pipeline's data origin. Land masking is also applied as a post-processing step at the final SIC output stage (see `prescient_ice_pipeline_architecture.md` Stage 5) so that the published SIC product clearly distinguishes land from predicted open water.

**Per-patch valid-fraction mask.** For each 8 × 8 patch, the fraction of source pixels that were valid (non-nodata and non-land) before substitution is computed and stored as a 32 × 32 float grid alongside the patch token output. This mask is the basis for downstream filtering: patches below a configurable validity threshold are excluded from the training dataset and from the final SIC product. Decoupling the filtering from the encoding step means the threshold can be tuned without re-running Clay, and different downstream consumers can apply different thresholds. The mask is serialised as a sidecar product on the same STAC item as the patch token COG — see `prescient_ice_pipeline_architecture.md` for the storage spec.
...
- **Per-patch valid-fraction threshold.** What fraction of valid (non-nodata, non-land) source pixels a patch must contain to be included in training and in the final SIC output. A placeholder of 80% is used in the design discussion; the operational value is empirical and should be revisited once the first end-to-end runs produce data on the coverage-versus-quality trade-off. Different thresholds can be applied per consumer (training versus final SIC product) without re-running Clay, since filtering is decoupled from embedding generation.
- **Land mask source for the 2025–26 Hudson Bay pipeline.** Whether to derive the project's `land-mask` STAC collection from GSHHG full-resolution coastlines, OSM coastlines, or another hydrography product. AI4Arctic training data uses the bundled `distance_map` variable directly and does not depend on this decision. The 2025–26 source should be no coarser than ~10m at the relevant latitudes to avoid quantisation artefacts at the 40m SAR pixel scale. Defer until the 2025–26 ingestion path is being implemented.
