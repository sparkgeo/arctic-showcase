...
| Edge-patch prediction reconciliation | **Open** | Backward-shifted edge chips (overlapping-edge-chip placement) produce overlapping patch predictions that can claim the same canonical 320m cell at SIC product assembly (A9 / Stage 5). The reconciliation rule (last-write-wins, mean, or highest valid-fraction) is unspecified and must be fixed before product assembly. See `prescient_ice_implementation_plan.md` § Open questions. |
