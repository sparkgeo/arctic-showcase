...
- **Dependencies:** P0.1.
- **Done when:** The per-band constants are computed across the 513 train-and-validation scenes (test scenes excluded) and stored at a fixed S3 location that both B3 (training encoding) and A8 (inference encoding) read, so the two paths normalise identically.
...
- **Inputs:** AI4Arctic NetCDF scenes (σ⁰ HH/HV, `btemp_FFP` AMSR2, ERA5 variables, `distance_map`, `sar_grid_latitude`/`sar_grid_longitude` GCPs, `sar_grid_points`); the bundled ice charts as labels for the 513 train/validation scenes, and the separately distributed post-challenge label file for the 20 test scenes.
- **Outputs:** A loader producing prepared 256-pixel chips on a regular grid with overlapping edge chips, with land treated as nodata (via `distance_map` code 0), nodata substituted by the per-band mean (B1), and a per-patch valid-fraction grid (valid pixels / 64 per 8×8 patch) computed before substitution. Fully-invalid chips skipped. For test scenes, the loader joins the separate label file so chart/label data is returned uniformly regardless of split.
...
### B3 — Custom `sentinel-1-ew` Clay metadata entry and encoding module
...
- **Outputs:** A custom `sentinel-1-ew` Clay metadata entry (HH index 0, HV index 1; NERSC mean/std from B1; GSD 40m), committed to a fixed, documented location for A8 to consume. An encoding module that takes a prepared chip and returns its patch tokens (32×32×1024), class token (1024-dim), and per-patch valid-fraction grid **in memory**. On the training path the module writes no COGs and registers nothing in Prescient — the B4 harness calls it inline and routes its output into the feature table. (The inference path, A8, wraps the same metadata entry and encoding logic but serialises to the `clay-embeddings` collection; that serialisation is A8's concern, not B3's.)
- **Dependencies:** B1, B2, P0.2.
- **Done when:** The encoder returns `[B, 1025, 1024]`; the EW HH/HV embedding dimension is empirically confirmed at 1024 (resolving the open question); index 0 is taken as the class token and indices 1–1024 reshaped via `einops.rearrange(encoded[:, 1:, :], "b (h w) d -> b d h w", h=32, w=32)`; the module returns embeddings in memory and is callable from the B4 harness.
- **Notes:** `mask_ratio=0.0` and `shuffle=False` enforced at model load; `model_size="large"` set explicitly (default "base" is 768-dim). The built-in `sentinel-1-rtc` entry (VV/VH, RTC, global stats) cannot be reused — hence the custom entry. The single-pass training design (no intermediate embedding COGs) is described in `prescient_ice_pipeline_architecture.md` § Stage 3 and § Training Dataset Assembly. The metadata entry and B1 constants are the parity-critical artefacts A8 must reuse rather than re-derive.

### B4 — Single-pass training dataset assembly (two-table GeoParquet)

- **Inputs:** The B3 encoding module (called inline); prepared chips and co-registered AMSR2 `btemp_FFP`, ERA5 variables, and distance-to-land index (B2); the bundled ice charts for label preparation (per `prescient_ice_training_strategy.md`).
- **Outputs:** Two GeoParquet tables in S3, written in a single pass over all 533 AI4Arctic scenes (513 train/validation + 20 test), partitioned by `scene_id`. The **patch-grain table** (one row per patch): patch token (1024-dim), raw HH/HV backscatter stats, AMSR2 channels, ERA5 variables, distance-to-land index, per-class label fraction vector (`frac_sic0` … `frac_sic10`), `valid_class_fraction`, discrete `label` (populated for every patch by the area-weighted collapse), `is_pure`, patch geometry, `scene_id`, `chip_id`, SAR `valid_fraction`, and `split` (train / validation / test). The **chip-grain table** (one row per chip): class token (1024-dim), chip geometry, `scene_id`, `chip_id`, `split`.
- **Dependencies:** B3 (encoding module), B2, P0.2 (GPU for the inline encoding pass).
- **Done when:** A single pass produces both tables over all 533 scenes, partitioned by scene; the patch table carries the feature-contract column schema and order; the `chip_id` join produces the correct Configuration 3 width (2069); `label` is populated for every patch by the area-weighted collapse (pure cells the degenerate case) and `is_pure` is set, with the fraction vector populated throughout; the test scenes' separately distributed labels are joined and prepared identically to the training labels; the scene-level train/validation/test split filters cleanly on `split` (a scene-level assignment, so no chip or patch crosses the boundary); geometry projects out at load time without entering the feature matrix.
- **Decomposition:** Implemented as three modules plus a harness — a label preparation module (area-weighted labels for every patch + per-class fraction vectors + `is_pure`), a provenance-agnostic feature assembly module (`assemble_rows`, shared in spirit with the A9 inference path), and a harness that calls the B3 encoder, the label module, and the assembly module over the corpus in one pass. GitHub issues break these out as an Ask cluster.
- **Notes:** Tables join on `chip_id` only for Configuration 3; Configurations 1 and 2 read the patch table alone. The patch table stores the fuller patch set down to a permissive `valid_fraction` floor (open design question — see § Open questions and `prescient_ice_index.md`), so `valid_fraction` threshold changes, the pure-only-versus-all-patches choice (a filter on `is_pure`), and any later revision of the mixed-cell labelling rule are read-time operations, never a re-encode. Label preparation, the fraction-vector scheme, and the area-weighted collapse follow `prescient_ice_training_strategy.md`. The single-pass, no-intermediate-COG rationale is in `prescient_ice_pipeline_architecture.md` § Training Dataset Assembly. Feature dimensions: Config 1 = 26, Config 2 = 1045, Config 3 = 2069. The chip-level fallback adds no stored columns — its features are read-time aggregations of the patch table joined to the chip table (see `prescient_ice_model_architecture.md` § Feature Configurations).
