...
- **Outputs:** Two GeoParquet tables in S3, written in a single pass over the AI4Arctic corpus, partitioned by `scene_id`. The **patch-grain table** (one row per patch): patch token (1024-dim), raw HH/HV backscatter stats, AMSR2 channels, ERA5 variables, distance-to-land index, per-class label fraction vector (`frac_sic0` … `frac_sic10`), `valid_class_fraction`, discrete `label` (null for unresolved mixed cells), patch geometry, `scene_id`, `chip_id`, and SAR `valid_fraction`. The **chip-grain table** (one row per chip): class token (1024-dim), chip geometry, `scene_id`, `chip_id`.
- **Dependencies:** B3 (encoding module), B2, P0.2 (GPU for the inline encoding pass).
- **Done when:** A single pass produces both tables, partitioned by scene; the patch table carries the feature-contract column schema and order; the `chip_id` join produces the correct Configuration 3 width (2069); pure-cell `label` is assigned directly and mixed-cell `label` is null with the fraction vector populated; the scene-level train/validation split filters cleanly on `scene_id`; geometry projects out at load time without entering the feature matrix.
- **Decomposition:** Implemented as three modules plus a harness — a label preparation module (Phase 1 pure-cell labels + per-class fraction vectors), a provenance-agnostic feature assembly module (`assemble_rows`, shared in spirit with the A9 inference path), and a harness that calls the B3 encoder, the label module, and the assembly module over the corpus in one pass. GitHub issues break these out as an Ask cluster.
- **Notes:** Tables join on `chip_id` only for Configuration 3; Configurations 1 and 2 read the patch table alone. The patch table stores the fuller patch set down to a permissive `valid_fraction` floor (open design question — see § Open questions and `prescient_ice_index.md`), so threshold changes and the Phase 1 → Phase 2 label expansion are read-time operations, never a re-encode. Label preparation, the fraction-vector scheme, and the Phase 2 midpoint collapse follow `prescient_ice_training_strategy.md`. The single-pass, no-intermediate-COG rationale is in `prescient_ice_pipeline_architecture.md` § Training Dataset Assembly. Feature dimensions: Config 1 = 26, Config 2 = 1045, Config 3 = 2069.
