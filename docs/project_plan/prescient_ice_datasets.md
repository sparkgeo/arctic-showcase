...
AI4Arctic ships in two noise-corrected versions of the Sentinel-1 data: the ESA-applied standard correction, and an additional **NERSC noise correction** developed by the Nansen Environmental and Remote Sensing Center specifically for EW-mode HV-channel sea ice work. The NERSC correction addresses residual scalloping (periodic intensity variations across the swath caused by antenna gain patterns) and incidence-angle-dependent biases that the ESA correction does not fully remove. These artefacts matter most on the HV channel, where the noise floor is closer to typical ice/water backscatter levels than on HH, and HV is also the channel most informative for ice/water discrimination. The NERSC-corrected version is therefore the project's preferred input — both for cleaner training statistics and to enable consistent application of the same correction to 2025–26 Sentinel-1 scenes acquired directly from CDSE for prospective evaluation and inference (see `prescient_ice_pipeline_architecture.md` for ingest-pipeline integration).

Two distribution forms are available. The **raw version** preserves original CT codes and provides direct access to the NERSC-corrected HV data — useful both for label preparation and for deriving normalisation statistics for Clay's custom `sentinel-1-ew` metadata entry. The **RTT (ready-to-train) version** pre-processes the data into a fixed input-output format aligned with the AutoICE challenge configuration. The raw version is preferred for this project because it gives more flexibility on label preparation strategy and direct access to the raw inputs needed for the custom Clay configuration.
...
**Category:** Model input. Coarse-resolution physics-based SIC prior, appended to patch token feature vectors.
...
**Access:** NSIDC via `earthaccess`; HDF-EOS5 format on NSIDC polar stereographic grids (EPSG:3411). For training, accessed via AI4Arctic NetCDF bundles.  
**Prescient collection:** `amsr2-sic` (COG) — for 2025–26 inference pipeline
...
Note that ERA5 includes its own sea ice concentration variable derived from passive microwave observations; this is not the same as the AMSR2 SIC product used elsewhere in this project and should not be used in its place.
...
For this project, the CDR is not used as a model input or training label — its 25 km resolution and retrospective processing cycle make it unsuitable for either role; the AMSR2 AU_SI12 product fills the SIC prior role with better resolution and latency. Its inclusion as a supplemental dataset is motivated by the long-term context it provides: visualizing four-plus decades of Arctic sea ice decline as a backdrop to the project's high-resolution SAR-derived output strengthens the scientific narrative considerably, and it demonstrates Prescient's ability to ingest and serve coarse gridded climate data records alongside fine-resolution derived products.
