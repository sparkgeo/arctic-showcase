...
- **Train and validation** are drawn from the **513 labelled training scenes**, partitioned at the scene level (the split is a filter on `scene_id`, so no chip or patch crosses the boundary). Training fits the downstream classifiers; validation drives model selection across the feature-configuration × classifier sweep (B5) and is the set on which the modelling Phase 2 go/no-go trigger is evaluated.
...
The single assembly pass (B4) processes all 533 scenes and tags each with its split, so a downstream consumer selects train, validation, or test by filtering a `split` column rather than maintaining separate tables. The test scenes' labels are joined from their separate label file at load time (B2) so that label preparation runs identically across all three splits.
